...
**Snapshot:** 341 tests green (unit+integration; +23 from Chunk 2a: synthesis logic, adjudication persistence, `/synthesis` command) · ruff format/check + mypy --strict + bandit clean · **CUDA/GPU enabled on the RTX box** (Windows; embeddings + reranker on `cuda:0`, retrieve+rerank ~68 ms).
...
| 6 — Per-project routing + Figures & Tables + Dual interpretation | 🔄 In progress | Reviewer agent ✅ + LLM provider protocol ✅ + Chunk 2a dual-interpretation ✅ (merged PR #1); Feature 4 detection layer (`regions.py`) done, engine = Marker; **4a Marker ingest build** + figure extraction (4b) next |
