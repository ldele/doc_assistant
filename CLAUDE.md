...
**Snapshot:** 353 tests green (unit+integration; +23 Chunk 2a, +12 Feature 4a: Marker table parse/splice + CI gate) · ruff format/check + mypy --strict + bandit clean · **CUDA/GPU enabled on the RTX box** (Windows; embeddings + reranker on `cuda:0`, retrieve+rerank ~68 ms).
...
**Next priority (RESUME HERE — 2026-06-04):** Chunk 2a dual-interpretation **shipped + merged (PR #1)** — live-verified end-to-end (query → segment → eager `answer_claims` → in-context accept/reject/edit). Benchmarks locked except the chunking sweep (deferred to RTX). Two open threads:
...
**B. Feature 4a — code ✅ shipped (commit `2933881`).** `tables_marker.py` (parse + page-anchored inline splice/de-dup) + `extract_tables_marker.py` (parallel, `regions`-gated CLI) + deterministic CI gate + opt-in `cases.tables.yaml`; 353 tests green, ruff/mypy/bandit clean. **Remaining is RTX-only validation (no more CPU code):** run `extract_tables_marker --apply` on the public corpus with `uvx` + surya models, confirm the `--paginate_output` flag/delimiter against the pinned marker-pdf, then run `cases.tables.yaml` (`--with-llm-judge`) and tweak its expected values to the real DPR table. After that, **Feature 4b** (figure detection + ordered objects-manifest) is the next code build.
