<!-- status: active · updated: 2026-09-16 (docs pass: rows 35/37/47/73/74 repointed at the frozen ui-checklist archive; 53/54/51 re-scoped against the code; KI-48 out of F1; dropped follow-ups captured as rows 76–89; row 46 built, one sandbox run left; S-1 + S-2 done, row 60 → S-3; KL1 built, A2 split to KL1b; rows 90–91) · class: living -->
...
| 60 | F10 Platform | **Security — one step per session.** `docs/security.md` §4 is the ordered plan (S-1 … S-12, then the full check §6); this row names the **current step** and moves each session. Foundations that landed 2026-09-10: gates cover `apps/`, `npm audit` in CI, the Tauri config guard test, loopback compose, the floor + threat model written down | in progress — **current step: S-3 sanitise `{@html}`** (S-1 size caps + S-2 walk cap done 2026-09-16) | `docs/security.md` §4 · `docs/reviews/REVIEW_2026-09-10_project-review.md` §4 |
...
| KL1 | F5 Knowledge | **Knowledge layer tells the truth** (Phase A). **Built 2026-09-16:** (A1) the stance chips read `contested? (experimental)`, the Settings toggle and GLOSSARY say "not a corpus measurement", `unsourced_claim` is shown as "Uncited in answers"; (A3) pieces that assert nothing — list lead-ins, headings, Sources blocks — no longer count as unsourced (`synthesis.is_claim_unit`, applied at the gap floor only); (A4) most of RG-014's grades were already in the list — what remained: `thin_bridge` flags the smaller side of a bridge between two groups, and "N open" counts only visible rows; (A5) `knowledge-layer.md` §6 re-read. **A2 moved to KL1b.** **Left:** a $0 `build_gaps --apply` so the stored rows reflect it (the user's go) | in progress — built; gap rebuild left | `docs/knowledge-layer.md` · ADR-040 · KI-33 · `docs/plans/PLAN_2026-08-03_knowledge-layer-to-goal.md` Phase A (local) |
| KL1b | F5 Knowledge | **Per-concept status — ADR-041 option 6, split out of KL1 (2026-09-16).** Option 6 says "re-base per-concept status on the claim layer", but the only claims in the tree are the assistant's answer sentences (`AnswerClaim`): a status built on them measures query history × model, reaches 9 of 13 graph concepts (none of the 3 `single_source` ones), and joins to concepts only at document level (`chunk_key` is not stored on citations). Decide first: corpus-side status only (document counts, presence, years), with answer-claim numbers shown separately as "in your answers"? | gate first — decision (amend ADR-041) | ADR-041 · REVIEW 2026-09-16 · `provenance.py` |
...
| 91 | F5 Knowledge | **Say when the gap list predates the graph** — gap rows carry the `graph_version` they were built from, a CLI skeleton rebuild leaves them behind (16 of 18 rows were two versions old on 2026-09-16), and the view never says so. Comparing the rows' version with the current skeleton's is a sound signal (unlike version-vs-vocabulary, which `GraphStaleness` rightly rejects); surface it beside the existing stale notice | planned (small) | `knowledge/concept_graph_view.py` `GraphStaleness` · `knowledge/gaps.py` |
