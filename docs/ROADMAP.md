<!-- status: active · updated: 2026-07-09 · class: living -->
...
| U2 | UI: chat layout — right-aligned, width-capped user bubble; RAG answer stays full-width, unbounded | **design-locked (grilled 2026-07-10)** — 1st in build order | `docs/specs/feature-phase8-ui-upgrade.md` |
| U3 | UI: citation side panel — click inline `[n]` to open a slide-over with that source's chunk detail (Chainlit-style); source cards no longer render inline by default | **design-locked (grilled 2026-07-10)** — 2nd in build order | `docs/specs/feature-phase8-ui-upgrade.md` |
| U1 | UI: Settings disclosure (surface the full read-only knob set) + build the ADR-010 RAG sandbox knobs + a manual System/Light/Dark theme toggle (persisted client-side, not a backend setting) | **design-locked (grilled 2026-07-10)** — 3rd in build order | `docs/specs/feature-phase8-ui-upgrade.md` |
| U1b | UI: Settings — add the two ADR-010 "must revisit" niche knobs (`EPISTEMICS_MARKERS_ENABLED`, `REVIEWER_EVIDENCE_CHARS`) to the sandbox surface | **design-locked (grilled 2026-07-10)** — 4th in build order, needs U1 landed first | `docs/specs/feature-phase8-ui-upgrade.md` (ADR-010 amendment 2026-07-10) |
| U1c | UI: Settings — provider/API-key management (Anthropic ↔ Ollama switch, key storage) | **scoped, NOT designed** — needs its own ADR before buildable; 5th in build order once that ADR lands | `docs/specs/feature-phase8-ui-upgrade.md` (stub only) |
