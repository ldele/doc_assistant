<!-- status: active · updated: 2026-09-20 (row 51 built: endpoint-kind validation, a deterministic `is_a` proposer, TX3b review in the app; S-4 done; row 93 brought forward to sequence 5 on the user's order) · class: living -->
...
| 5 | **93 — meanings are curated (ADR-052): a place to read and edit a definition, suggestions from the corpus, every write undoable and comparable with the label-only count** (user's order, 2026-09-20 — brought forward past KL4) | S-5 host guard |
