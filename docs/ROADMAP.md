...
1. **Now — the gaps, three strands in parallel, one small security step every session.**
   ...
   - *The knowledge layer made to work:* **KL1** (tell the truth — done 2026-09-17) → **53 + 54** (merges that keep
...
| 51 | F5 Knowledge | **Concept hierarchy for concepts** — `is_a` edges among curated concepts (ADR-019/ADR-028 machinery is built; the taxonomy is *empty*: 13 of 357 concepts placed, 0 `is_a`, ADR-045) + **TX3b** in-app accept/reject of proposed placements. Raised by the user three times. **Scope found 2026-09-16:** nothing can write an `is_a` edge except a raw API POST — the taxonomy view and auto-propose write `in_field` only — and `add_hierarchy_edge` checks cycles but not endpoint kinds (`is_a` concept→field is accepted). So this row needs kind validation and an `is_a` propose/accept path, not TX3b alone; `write_proposals` catches only `ValueError`, so one integrity error loses the whole batch. The router SQL move in row 83 can ride here **Input found 2026-09-17:** 21 of the 34 bge-base merge pairs at ≥ 0.85 are narrower terms — `is_a` candidates, not duplicates (merge baseline). **Built 2026-09-20:** `add_hierarchy_edge` refuses an endpoint kind that does not match the edge type (`EdgeKindError`, 400 at the API); `knowledge/isa_propose.py` + `scripts/propose_isa.py` propose `is_a` deterministically from a shared head ($0, dry-run default, **not yet run with `--apply` on the library — the user's call**); `GET /api/taxonomy/proposals` + the taxonomy modal's *Proposed placements* pane give accept/reject to every proposal, and `attach_document_field` gained the promote that made accepting a document classification possible; `write_proposals` no longer loses a batch to one stale id. **Measured:** 27 candidates of 357 concepts, first reading 17 ok / 5 check / 5 fragment, **1 touching the 13 graph concepts** (`tests/eval/baselines/isa_head_suffix_2026-09-20.md`) — a shared-head rule does not build a spine *for the graph*. **Left:** the user's go on `--apply`, then the review; the 21 narrower merge pairs whose direction is not lexical still need the hand score (row 53 (3)); the row-83 router SQL move did not ride here | built — review left to the user | ADR-028 · ADR-045 · `docs/specs/feature-taxonomy-auto-propose.md` · `tests/eval/baselines/isa_head_suffix_2026-09-20.md` · REVIEW 2026-09-16 §1 C-3/C-7 (local) |
| KL1b | F5 Knowledge | **Per-concept status — ADR-041 option 6, split out of KL1 (2026-09-16).** Option 6 says "re-base per-concept status on the claim layer", but the only claims in the tree are the assistant's answer sentences (`AnswerClaim`): a status built on them measures query history × model, reaches 9 of 13 graph concepts (none of the 3 `single_source` ones), and joins to concepts only at document level (`chunk_key` is not stored on citations). Decide first: corpus-side status only (document counts, presence, years), with answer-claim numbers shown separately as "in your answers"? **User leans no (2026-09-17):** the gap list already classifies knowledge per concept (`single_source`, `isolated`, "Uncited in answers"), so a separate status would restate it. If confirmed: close this row and amend ADR-041 — option 6's buildable part is the gap list; the disagreement half stays KL3 | gate first — decision (user leans no; amend ADR-041 on confirm) | ADR-041 · REVIEW 2026-09-16 · `provenance.py` |
...
| 53 | F5 Knowledge | **Concept merges: make them safe, then measure the threshold** (crossover §3.2; **re-scoped 2026-09-16** — checked against the code, the original row swept a knob that never drives a merge). **(1) A merge must not destroy curation.** `apply_merges` folds aliases and deletes the dropped `Concept`; `concept_hierarchy`'s `ON DELETE CASCADE` takes that concept's placements with it, and `gap_triage` (no FK) is orphaned; nothing is logged. KI-20 (2026-07-21) kept merges as fold-and-drop because aliases carry the vocabulary — before `concept_hierarchy` existed. Re-point placements to the survivor through `add_hierarchy_edge` (cycle-checked), re-key triage, log each merge so it can be inspected and reversed. **(2) One definition of "the same concept."** The preview (`suggest_concepts --near`: `CONCEPT_MERGE_COSINE` 0.85, specter2, label + definition) and the destructive apply (`curate_concepts --dedup --apply`: a hard-coded 0.9, bge-base, label only) differ in threshold, model and input, so the preview does not predict the merge. **(3) Then sweep that one threshold** — 0.80 / 0.85 / 0.90, hand-scored on the pairs that change → `tests/eval/baselines/concept_merge_cosine_<date>.md`. Lands before row 51 writes curated `is_a` edges. **(1) + (2) built 2026-09-17:** `knowledge/concept_merge.py` moves placements, triage and surface forms to the survivor, refuses a cycle before writing, records each merge (`concept_merges`) and undoes it (`curate_concepts --merges` / `--undo-merge`); preview and merge share `dedup_pairs` at `CONCEPT_MERGE_MODEL` bge-base / `CONCEPT_MERGE_COSINE` 0.90 — the old merge's values, because the preview's (SPECTER2, 0.85) would merge 354 of 357 labels into one. **(3) measured, not scored:** bge-base at 0.85 → 34 pairs, ~7 duplicates / 21 narrower / 6 related by a first reading (`tests/eval/baselines/concept_merge_cosine_2026-09-17.md`). **Left:** the user's hand score (review page, in progress), then the design — **user's direction 2026-09-17: a threshold is not enough, concepts need context pairing**: lexical pre-pairing (inflection folded as surface forms, derivation such as `virus`/`viral` kept as a family), a context check (a fragment's stands-alone count separated fragments cleanly; a masked context embedding did not), review before any write — **ADR-052 (accepted 2026-09-18): a concept is one meaning** | in progress — hand score (user) | crossover §3.2 · REVIEW 2026-09-16 §1 C-1/C-2 (local) · `knowledge/concept_curation.py` `apply_merges` · `scripts/curate_concepts.py` · `config.py` `CONCEPT_MERGE_COSINE` · KI-20 (archive 002) |
| 90 | F3 Chat | **An unreadable citation form counts as uncited** — answers cite "According to Source 6: file.pdf" (45 of 1,239 unsupported pieces), which `_CITATION_TOKEN_RE` does not resolve, so a cited sentence is marked `unsupported` and inflates "Uncited in answers". Widening the parser moves markers on every answer **and the RG-012 citation verdict** — measure it against the stored answers ($0: re-segmenting reproduces 1,910/1,911 markers) before changing it | gate first — measure | `synthesis.py` · KL1 (DEVLOG 2026-09-16) · RG-012 |
| 91 | F5 Knowledge | **Say when the gap list predates the graph** — gap rows carry the `graph_version` they were built from, a CLI skeleton rebuild leaves them behind (16 of 18 rows were two versions old on 2026-09-16; rebuilt 2026-09-17), and the view never says so. Compare the **deterministic** rows only: stochastic `suggested_concept` rows keep their status and their old version across a rebuild by design (2 today). Comparing the rows' version with the current skeleton's is a sound signal (unlike version-vs-vocabulary, which `GraphStaleness` rightly rejects); surface it beside the existing stale notice | planned (small) | `knowledge/concept_graph_view.py` `GraphStaleness` · `knowledge/gaps.py` |
| 92 | F5 Knowledge | **Presence is sense-blind** — one label, several meanings, all counted as one concept: `viral` is an infection, a lab vector and a web meme; `specter` is a model and a book title; `din` a neuron type and "vitamin D in" split by extraction; `beta` an oscillation band, a code variable and a split "beta secretase" (merge baseline 2026-09-17, from the passages). Presence feeds `single_source`, the trustworthy signal, so a homograph can hide a gap or invent corroboration. **Measured 2026-09-18:** a document-level split (document vectors, the library's own 10th-percentile similarity as "unrelated") flags 3 of 162 multi-document concepts — `specter` (true homograph), `abstractions`, and `virus` (same sense, another domain: must not split); within-domain senses (`viral`, `beta`, `din`) need passage context. **Decided in principle (user, 2026-09-18): a different meaning is a different concept** — ADR-052 (accepted): a shared word gets a scope (document, else passage), found from evidence and confirmed in review; every step reversible and comparable with the label-only count | planned — after 93 | `knowledge/concept_skeleton.py` `match_presence` · `tests/eval/baselines/concept_merge_cosine_2026-09-17.md` |
| 93 | F5 Knowledge | **Meanings are curated (ADR-052)** — the build the ADR commits to: a place in the app to read and edit a concept's definition (none exists — 2 of 357 concepts have one, set by CLI); definition suggestions from the corpus, marked until accepted; a disambiguating qualifier wherever two concepts share a label; the label-keyed paths (`add_concept`, `promote_keyword`, `create_keyword_family`, the rename clash check, the one-family-per-keyword rule) made meaning-aware; then the scope step in presence for shared words (row 92). Every write recorded and undoable; the label-only count kept beside the meaning-scoped one, with the difference shown (the user's condition). Graph concepts and flagged shared words first. **Source material is thin:** a strict scan for defining sentences ("X is a …", "X refers to …", "… defined as X") finds one for 2 of the 19 priority concepts (2026-09-18) — the library's papers use these terms without defining them, so definitions need surveys and reference papers added to the corpus. **Six added 2026-09-18** (`tests/eval/baselines/new_papers_definitions_2026-09-18.md`): 5 gaps closed, no graph concept single-source any more; a concept's **first sentences in a survey** found an introducing passage for every priority term the patterns missed — the suggestion source to build on; alias meaning problems surfaced (`contrastive`, `negative sampling`, two definitions of `hard negatives`)  **Shape decided 2026-09-21 (ADR-053, proposed — the user's direction): a definition is chosen from candidates that keep their source, none overwrites another** — a verbatim passage, the user's own words, a model's text with the inputs it was handed (passages, or reference titles alone), each with its reliability as evidence; thin evidence becomes papers to add. **93a built 2026-09-21:** `knowledge/definitions.py` (candidates + choose / dismiss / restore / undo, the only writer of `Concept.definition`), passage extraction (`scripts/extract_definitions.py`, and "Look in my library" in the panel through the keyword index — 12 s → 0.3 s, identical to a full read on 357 of 357), the definitions API, the card in the Graph tab's concept panel, and a vocabulary search that reaches concepts off the graph; merges carry candidates and undo returns them; the two old definitions migrate as chosen candidates at boot. **Measured:** 327 of 357 concepts get candidates, 37 a strong one; on the 19 priority concepts the grade agrees with a by-hand reading 18 of 19 (`tests/eval/baselines/definition_sources_2026-09-21.md`). **Not run on the library:** `--apply` — the user's call. **Expert vocabularies added as a source (user, 2026-09-21):** a probe found curated definitions for 8 of the 19 priority concepts (MeSH, the OBO ontologies, the mouse-line registry) and one-line glosses for 6; the youngest retrieval terms have none (`tests/eval/baselines/reference_vocabularies_2026-09-21.md`). Local copies picked by the document's field, matched with the library as judge, shown as *what the field says* beside *how your library uses it*. **Left:** 93b model candidates (from passages; from reference titles alone) with a grounding check · **93c expert vocabularies** (local copies, matcher, `reference` candidates, the two-layer panel) · 93d what the references say + papers to add · 93e the shared-word split review (row 92). The hand labels on the Definition Candidate Review page are the yardstick for all of them. **Labelled 2026-09-22** (69 candidates, grade hidden): usable 14/69; strong 8/13, first mentions 3/45. **The user decided** (ADR-053 amendment): first mentions become *how your library uses it*, not candidates (**built** — usable 11/24 among what remains); *claim* becomes a flag in the next labelling round; labels keep their written case (next); an abbreviation / fragment signal with reasons, checked outside the 19 | **93a built**; the 2026-09-22 refinements — written case next, then 93b | ADR-052 · ADR-053 · `knowledge/definitions.py` · `tests/eval/baselines/definition_sources_2026-09-21.md` · `tests/eval/baselines/reference_vocabularies_2026-09-21.md` · `tests/eval/baselines/definition_labels_2026-09-22.md` |
...
vocabulary curation (row 23), KL1 — the signals say what they measure (2026-09-17; DEVLOG
2026-09-16 (5); "Uncited in answers" stays a gap kind, user 2026-09-17), row 54 — no write can
put a field node on the graph, and coverage counts what it describes (DEVLOG 2026-09-17 (1)). **Read `docs/knowledge-layer.md` §6 before believing any number:**
