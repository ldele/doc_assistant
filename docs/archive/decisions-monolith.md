<!-- status: archived · updated: 2026-07-19 · class: append-only -->

# Design decisions (pre-cpc monolith — FROZEN)

> **Archived 2026-07-19 (ADR-022).** This is the pre-cpc decision record (2026-05 → 2026-07-16),
> moved verbatim from `docs/decisions.md` and frozen. ADR-001 Step 4 planned a full split into
> per-file ADRs; ADR-022 re-scoped that: decisions made since cpc adoption already live as
> `docs/decisions/ADR-NNN-*.md`, phase-status blocks below are superseded by `docs/ROADMAP.md`,
> and the remaining rationale stays addressable here (the cpc route check resolves the old
> `docs/decisions.md` references into `docs/archive/`). The live index is `docs/decisions.md`.
> Do not edit this file; new decisions get a new ADR.
