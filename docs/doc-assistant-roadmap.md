...
- 7b — ✅ **Louvain** communities (networkx-native; Leiden deferred for Windows-wheel safety) + god-node ranking. The bridge back to Feature 6 (`concept_graph.doc_clusters_from_graph`) is built and **✅ wired into the wiki (2026-06-17)**: `wiki.load_communities` clusters documents by these communities behind `WIKI_USE_CONCEPT_COMMUNITIES` (inert by default, freshness-checked, cosine fallback). Validated free — cosine@0.90 collapsed the 10-paper corpus to 2 topics; concept communities gave 9. Deferred refinement: derive the `[[links]]` from inter-community concept edges (still doc-cosine edges today) + flip the flag default. See decisions.md → Deferred Improvements.
- 7c — ✅ Graph-structure gap signals (isolated nodes, thin bridges) — complements Feature 6's citation-thinness signals.
- 7d — ✅ **Knowledge-currency / claim-corroboration layer (engine shipped 2026-06-17, validated free on local Ollama).** Claim-level structural weights (corroboration, contradiction, supersession direction — **age is not an input**), projected onto chunks as a `chunk_epistemics` sidecar; reviewer `contested_evidence` tag added. Unique-source claims are neutral, never penalized (held on the real run: 169/198 nodes `unique`). `concept_graph.py` extended with relation polarity + per-doc `EdgeSupport`; new `epistemics.py` + `scripts/compute_epistemics.py`. **Deferred follow-ups:** live answer-time marker injection into the synthesis evidence layer (needs a stable chunk key plumbed through retrieval; synthesis untouched this PR so eval stays byte-identical) and the `query_router.py` seam (Decision 8). No retrieval-rank integration in v1 (eval-gated if ever). Spec: `docs/specs/feature-7d-knowledge-currency.md`.
