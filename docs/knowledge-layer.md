<!-- status: active Â· updated: 2026-08-05 Â· class: living -->

# The knowledge layer â€” what the concept graph is for, and which of its signals you can trust
...
[`architecture.md`](architecture.md) Â§ *Concept & knowledge system*; the design record lives in
`docs/specs/feature-concept-graph.md` and the ADRs cited per section. Where they disagree with this
page, they win â€” but tell this page.

> **Why this page exists.** The purpose was written down (in a feature spec), the mechanism was
> written down (in architecture.md), and the decisions were written down (across nine ADRs) â€” but
...
From `feature-concept-graph.md` Â§ *The job* (locked with the user 2026-07-17) â€” **three questions,
one surface**:

1. **Corroboration** â€” *"is this concept backed by more than one source?"* In the user's words:
...
2. **Coverage** â€” *"have I read the field?"* A PhD student must cover a literature; a professor
...
the LLM) cannot see â€” concepts the corpus under-supports, claims it cannot source, and directions
...
> documentation. â€¦ The most barebone goal of the graph feature is to be able to classify knowledge
...
> easier. â€¦ We want epistemics feature. That is the idea."*
...
2. **The unit is the concept.** Knowledge classified *per concept*. Note that today's epistemics
   classifies **edges** (concept pairs) and reaches concepts only by aggregating them â€” a unit
...
3. **A gap is a deviation from expected structure, not an absolute count.** The user's framing:
   *concepts are linked in general to predictable things.* That is what makes "missing" detectable â€”
...
documents** â€” corroboration is `len(doc_ids) >= 2`, coverage is presence per field, navigation is
`node â†’ doc_ids â†’ chunk_keys` â€” so the *skeleton* needs no LLM, and RG-014 graded `single_source`, a
document count, as the one detector that is a *"TRUE POSITIVE â€” the product thesis"*. **That is an
...
## 2. The vocabulary â€” one table, two opt-ins
...
| `graph_include` | **opt-in** â€” this concept participates in the graph (ADR-018) | CLI only; the graph UI is read-only (ADR-017) |
...
## 3. Two graph layers over the same nodes â€” never conflate them
...
rebuild would wipe it â€” the KI-17 / KI-20 class of bug.

**Node A is zero-LLM** â€” co-occurrence â‰¥ 2 plus boundary presence (ADR-008), deterministic and
...
annotations â€” rebuild with `--apply --enrich` together.
...
flowchart TD
    ...
    CON --> NA["Node A â€” concept_skeleton.py<br/>co-occurrence + Louvain Â· ZERO LLM (ADR-008)"]
    NA --> SK[("skeleton.json<br/>nodes Â· edges Â· communities")]
    NA -.optional.-> NB["Node B â€” concept_skeleton_enrich.py<br/>LLM relation + stance"]
    NB -.-> SK
    SK --> GAPS["gaps.py â€” deterministic detectors<br/>single_source Â· isolated Â· thin_bridge Â· under_connected"]
    SK --> EPI["epistemics.py â€” node weights<br/>projected onto chunks"]
    SK --> WIKI["wiki.py â€” clusters over communities"]
    CON --> TAX["taxonomy.py â€” curated is_a / in_field DAG (ADR-028)"]
...
| **Epistemics markers** (`epistemics.py`, ADR-027) | node `coverage` / `direction` â†’ `chunk_epistemics` â†’ the strip and the answer layer | **only via Node-B stance** |
| **Wiki / synthesis** (`wiki.py`) | Louvain communities | for the note text |
| **Graph UI** (ADR-017) | the whole skeleton, read-only | no |

---

## 6. Trust status â€” read this before believing a number

Signals in this layer are **not** equally sound. As of 2026-08-03:

| signal | status | why |
|---|---|---|
| **`single_source`** | âœ… **trustworthy â€” the product thesis** | a document count; RG-014 graded it a true positive |
| Concept presence / navigation | âœ… trustworthy | 1781/1781 chunk keys resolved live |
| Communities, co-occurrence edges | âœ… deterministic | Node A, seeded Louvain, idempotent |
| `unsourced_claim` | âš ï¸ real but **~33% contaminated** | markdown headings counted as claims; never present the count as precise |
| `thin_bridge` | âš ï¸ redundant / half-misleading | flags both endpoints, so the most-connected node gets called a thin bridge |
| `under_connected` | âŒ **noise at small vocabularies** | measures graph degree, dominated by vocabulary sparsity, not corpus coverage. Do not show by default |
| **`contested` / `superseded_trend`** | âŒ **NOT A CORPUS MEASUREMENT (KI-33)** â€” and **withheld from the UI since v0.4.1** | see below |
...
pair list â€” never the document text* it is asked to judge "the apparent framing" of. Its label set
...
same 17 pairs, temperature 0, varying only position â†’ four different verdicts crossing the
...
produce one-pair prompts and "supports" â€” which is exactly the measured confound (**7 of 9** concepts
contested in Machine learning + Data management, **0 of 4** outside).

Measured: [`node_b_stance_validity_2026-08-02.md`](../tests/eval/baselines/node_b_stance_validity_2026-08-02.md) Â·
...
a threshold firing at one disputing document. Measured, that threshold moves density 53.3% â†’ 53.0%.
...
out with the reason at the line â€” markup and CSS together, so restoring is one contiguous uncomment.
...
that are sound â€” document year, relevance score, graph freshness. Nothing was deleted; the wire
...
| capability | state |
|---|---|
| **C1 â€” which claims are unsubstantiated** | âš ï¸ **working on the answer path, not reused per concept.** `AnswerClaim` + `weakly grounded`/`unsupported` are sound; `unsourced_claim` is real but ~33% contaminated |
| **C2 â€” classify knowledge per concept** | âŒ **invalid** â€” depends on Node-B stance (KI-33). Also a unit mismatch: today's epistemics classifies *edges*, the goal asks for *concepts* |
| **C3 â€” expose gaps** | âœ… **well covered** â€” `single_source` is the graded true positive; graph, ego view, gap list + triage all shipped |
| **C4 â€” acquisition direction** *("for subject X, go read Y")* | âŒ **no sound implementation anywhere.** Every built detector looks *inward* at the corpus; ADR-004 deferred the outward reach as Tier-2b and ADR-032 is still a stub |
| **C5 â€” research navigation** | âœ… mostly â€” graph, Connections, taxonomy view. âŒ the **per-document map** (MM1â€“MM3, PR-G2c) is planned, gated on ADR-030, never built |
...
2. **The one suggestion engine runs on the one detector graded noise.** `gap_suggest` â€” the closest
   thing to C4 in the tree â€” fires one LLM call per **`under_connected`** concept, the kind RG-014
   graded âŒ noise, while **`single_source`**, the kind it graded *"the product thesis"*, gets no
   suggestion pass at all. Re-pointing it is the cheapest real progress toward the goal.

**The plan** â€” phased so nothing is built on top of something that lies â€” is in
`docs/PLAN_2026-08-03_knowledge-layer-to-goal.md` (local-only, ADR-029), and its work items are
mirrored as ROADMAP rows **KL1â€“KL4**.
...
explicitly** (`CONCEPT_SKELETON_LLM_PROVIDER`), never inheriting `LLM_PROVIDER` â€” that is the KI-4
