<!-- status: active · updated: 2026-09-17 (S-3 done: DOMPurify on the one sink, the dev server sends the CSP — devCsp would not have; S-4 next) · class: living -->
...
| S2 | **LLM output is rendered as raw HTML** — `marked.parse` → `{@html}`, no sanitiser; `marked` ≥ 14 no longer filters `javascript:` | `apps/desktop/src/lib/chat/Markdown.svelte` | T1: a quoted passage carries markup into the DOM. The CSP contains it in the packaged app; the dev loop had **no CSP** — Tauri injects it only into assets it serves, and `tauri dev` loads the Vite server directly (**fixed 2026-09-17**, S-3: the dev server sends it). Residual even with CSP: `<form action>` / `<a href>` exfil and UI spoofing (`form-action`, `base-uri`, `object-src` do not inherit from `default-src`) | S-3 **done 2026-09-17**, S-4 |
...
| S-4 | **CSP residual** (S2): append `form-action 'none'; base-uri 'none'; object-src 'none'`; extend `test_desktop_security_config.py` | small | the test asserts the three directives; the installed build still loads pages and figures | **next** |
