...
| 5 | **Page-anchored inline replacement** (de-dup + placement). For each Marker table on page N: locate the `<!-- page:N -->`…`<!-- page:N+1 -->` span, strip pymupdf4llm's lossy GFM table(s) **within that span only**, insert Marker's wrapped in idempotent `<!-- table:marker:page=N:begin/end -->` markers. |
...
**Build-time confirmations (shell-out, so verify against the pinned marker-pdf version):** the `--paginate_output` flag name + the page-delimiter regex; pin the marker-pdf version in the CLI's docstring.
...
- `parse_marker_tables(marker_markdown: str) -> list[ExtractedTable]` — split paginated Marker markdown by page delimiter, extract GFM table blocks per page, light-filter, return `ExtractedTable(page, index, rows)` **but keep raw markdown** (see below). Reuses `tables.ExtractedTable` or a `MarkerTable` carrying the raw rich-markdown string (don't round-trip through `rows` if it would drop `<br>`/bold — preserve Marker's block verbatim).
- `splice_tables_inline(markdown: str, tables: list[...], *, engine: str = "marker") -> str` — page-anchored inline replacement using the `PAGE_MARKER` regex (import from `ingest`, or lift the pattern). Idempotent via the `<!-- table:{engine}:page=N:begin/end -->` wrapper.
...
**Files owned:** `src/doc_assistant/tables_marker.py` (new), `scripts/extract_tables_marker.py` (new), `src/doc_assistant/config.py`, `tests/unit/test_tables_marker.py` (new), `tests/integration/test_marker_table_retrieval.py` (new), `tests/eval/cases.tables.yaml` (new), `.env.example`, `docs/figures-and-tables.md` (update engine table).
**Status:** pending.
