"""Compute the gap-detection sidecar — Tier 1 + the Tier-2a deterministic floor
(ADR-004 / docs/specs/feature-gap-detection.md).
...
current skeleton + claims; any stochastic rows (the deferred Tier-2a ceiling,
`gap_suggest.py` — not built yet) persist their `status` across the rebuild.
...
# concept below this many edges routes into the (separate, not-yet-built) Tier-2a
# stochastic ceiling as a "thin" candidate.
...
def _format_report(result: GapsResult) -> str:
    ...
    if result.applied:
        out.append(f"Rows written:              {len(result.gaps)}")
    out.append("=" * 76)
    if by_kind:
        out.append("")
...
def main() -> int:
    ...
    parser.add_argument(
        "--suggest",
        action="store_true",
        help="Tier-2a stochastic ceiling (NOT built — raises; a separate, "
        "quarantined-LLM sprint).",
    )
    args = parser.parse_args()

    try:
        result = build_gaps(apply=args.apply, min_degree=args.min_degree, suggest=args.suggest)
